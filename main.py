...
class Main(QWidget):
    def __init__(self):
        super().__init__()
        global name_table_main
        manual = 'У Вас открыто два окна в этом окне ' \
                 'Вы видетеполя для ввода пути к файлу\n' \
                 'куда хотите сохранять, ввести файл ' \
                 'нужжно без разширения\n' \
                 'А во втором окне калькуляторе ' \
                 'Вы можете ввести выражение и нажать на кнопку prov\n' \
